...
class Test(test.Test):
	...
	def test(self):
		for meshFile in self.mesh_list:
			numX, numY, numZ = self.extractSizeFromMeshName(meshFile)
			dims = [numX, numY, numZ]
			if self.verbose:
...
if __name__ == "__main__":
	from meshPartitioner.mesh3D.meshTools3D import MeshTools3D
	from meshPartitioner.mesh3D.meshTools3D import TYPE_FLOAT
	
	meshTools = MeshTools3D(TYPE_FLOAT, 5)
